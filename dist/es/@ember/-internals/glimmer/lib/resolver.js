    ...
    _lookupComponentDefinition(_name, meta) {
        ...
        if (GLIMMER_CUSTOM_COMPONENT_MANAGER && component && component.class) {
            ...
            if (managerId) {
                ...
                assert(`Could not find custom component manager '${managerId}' which was specified by ${component.class}`, !!manager);
                let definition = new CustomManagerDefinition(name, component, manager, layout || meta.owner.lookup(P `template:components/-default`));
                finalizer();
                this.componentDefinitionCache.set(key, definition);
                return definition;
            }
